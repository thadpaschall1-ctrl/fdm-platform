...
export function DemoWidget({ popularNiches }: DemoWidgetProps) {
  ...
  const handleStartBrowserDemo = useCallback(async () => {
    ...
    try {
      // Initialize the ElevenLabs conversation via our API
      ...
      // Open ElevenLabs widget in a new window or iframe
      if (data.widget_url) {
        window.open(data.widget_url, "ai-demo", "width=400,height=600");
        setState("active");
      } else if (data.conversation_url) {
        window.open(data.conversation_url, "ai-demo", "width=400,height=600");
        setState("active");
      } else {
        setState("ready");
      }
    ...
    <div className="space-y-8">
      ...
      <div>
        <h2 className="mb-4 text-lg font-semibold text-white">
          <span className="mr-2 inline-flex h-7 w-7 items-center justify-center rounded-full bg-blue-600 text-sm font-bold">
            1
          </span>
          ...
          {popularNiches.map((n) => (
            <button
              ...
              onClick={() => {
                setNiche(n);
                setCustomNiche("");
                if (state === "select") setState("ready");
              }}
        ...
        <div className="mt-4">
          <input
            ...
            onChange={(e) => {
              ...
              setNiche("");
              if (e.target.value.trim() && state === "select") setState("ready");
            }}
          ...
          <div className="rounded-2xl border border-white/10 bg-slate-900 p-6">
            {selectedNiche && (
              <p className="mb-4 text-sm text-slate-400">
                Demo configured for:{" "}
                <span className="font-semibold text-blue-400">{selectedNiche}</span>
              </p>
            )}
            ...
            >
              {state === "connecting"
                ...
                : state === "active"
                ? "✓ Demo Running — Check Your Browser"
                : "🎙 Start Live Voice Demo"}
            </button>

            {state === "active" && (
              <p className="mt-3 text-center text-sm text-slate-500">
                A new window opened with the AI conversation. Talk to Holland!
              </p>
            )}
              ...
              {!showPhoneOption ? (
                ...
                <div className="space-y-3">
                  <p className="text-sm text-slate-400">
                    Enter your number — Holland will call you in about 10 seconds:
                  </p>
                  ...
                  {callState === "sent" && (
                    <p className="text-sm text-green-400">
                      Your phone should ring in a few seconds. Answer and talk to Holland!
                    </p>
                  )}
                </div>
              )}
